...
- [x] Vercel project set up (`vercel.json`, `api/` directory)
- [x] `api/advice.js` — serverless function: receives roster context, calls Gemini, returns response
- [x] `GEMINI_API_KEY` added to Vercel env vars (from Google AI Studio)
- [x] Install `@google/generative-ai` SDK
- [x] The Warden system prompt wired to `gemini-1.5-flash`
- [x] Draft Advice: real-time pick recommendation based on roster needs + available pool
- [x] Free Agent Advice: weekly waiver targets ranked by positional need + projected stats
- [x] Lineup Advice: daily "start/sit" based on injury status + matchup
- [x] "Deep Dive" button on any player → Gemini with grounding/search for recent news
- [x] Cost guardrail: warn if >1,400 advice calls/day (localStorage counter, well under free tier limit)

---

## Phase 3B — Draft Room Enhancements
_Warden pick suggestions + image prompt library._

- [x] `WardenPickSuggestion.jsx` — top 3 draft recommendations with player cards
- [x] Integration into `DraftWarRoom.jsx` (right column, above needs grid)
- [x] Auto-filter on DRAFT button click
- [x] Warden's 1-line scouting reports (static, can enhance with Gemini)
- [x] `IMAGE_PROMPTS.md` — 30+ ready-to-use image generation prompts (gitignored)
- [x] Prompts for: characters, logos, banners, badges, card backs, UI decorations
...
- [x] Deploy to `sportsball.utilityinfielder.com` via Vercel
